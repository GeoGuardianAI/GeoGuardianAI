...
def test_available_teams_are_returned() -> None:
    teams = rescue_team_service.get_available_teams(12.9716, 77.5946)
...
def test_specialization_filtering_works() -> None:
    teams = rescue_team_service.get_available_teams(12.9716, 77.5946, specialization="search")
...
def test_no_matching_specialization_returns_empty_list() -> None:
    teams = rescue_team_service.get_available_teams(12.9716, 77.5946, specialization="nonexistent")
    assert teams == []
...
def test_invalid_latitude_raises_value_error(latitude: float) -> None:
    with pytest.raises(ValueError, match="latitude must be between -90 and 90"):
        rescue_team_service.get_available_teams(latitude, 77.5946)
...
def test_invalid_longitude_raises_value_error(longitude: float) -> None:
    with pytest.raises(ValueError, match="longitude must be between -180 and 180"):
        rescue_team_service.get_available_teams(12.9716, longitude)
...
def test_location_near_known_mock_team_returns_that_team_first() -> None:
    ...
    near_bengaluru = rescue_team_service.get_available_teams(12.9716, 77.5946, specialization="mountain rescue")
    assert near_bengaluru[0].team_id == "rescue-ny-01"
